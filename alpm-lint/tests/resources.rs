...
/// given path. All functions have the same function signature.
mod factories {
    use super::{
        Path,
        TestResult,
...
fn multi_file_missing_files(
    ...
    assert!(
        result.is_err(),
        "Expected error when files are missing for scope {:?}",
        scope
    );
...
fn gather_follows_symlinks() -> TestResult<()> {
    ...
    let symlink = tempdir_path.join("symlink");
    unix::fs::symlink(&srcinfo_path, &symlink)?;

    let resources = Resources::gather(&symlink, LintScope::SourceInfo)?;
...
fn gather_follows_dir_symlinks() -> TestResult<()> {
    ...
    unix::fs::symlink(&source_dir, &symlink)?;

    let resources = Resources::gather(&symlink, LintScope::SourceRepository)?;
