...
For general best practices refer to the project's global [CONTRIBUTING.md](../CONTRIBUTING.md).
...
`LintIssue`s and its [`LintIssueType`] are converted to [`LintIssueDisplay`] first before being printed, to ensure a consistent formatting across the whole linting framework.
