...
/// For example, selecting [`LintingScope::SourceInfo`] will run all SourceInfo specific linting
...
impl LintScope {
    ...
    /// directory.
    ///
    /// Usually, when calling `alpm-linting check`, [`LintingScope::detect`] is used to
    ...
    /// Check whether the [`Resources`] is a single file.
...
mod tests {
    ...
    fn fail_to_detect_scope_in_directory(#[case] files: Vec<&'static str>) -> TestResult<()> {
        ...
        assert!(
            ...
            "Expected 'NoLintingScope' error for file set {files:?}"
        );
