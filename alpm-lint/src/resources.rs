...
impl Resources {
    ...
    /// # Errors
    ///
    /// - Files that are required for a scope dont' exist.
    ...
    pub fn gather(path: &Path, scope: LintScope) -> Result<Self, Error> {
        ...
        let resource = match scope {
            ...
            LintScope::SourceRepository => Resources::SourceRepository {
                package_build_source_info: SourceInfo::V1(SourceInfoV1::from_pkgbuild(path)?),
                source_info: SourceInfo::from_file_with_schema(path, None)?,
            },
            LintScope::Package => Resources::Package {
                package_info: PackageInfo::from_file_with_schema(path, None)?,
                build_info: BuildInfo::from_file_with_schema(path, None)?,
            },
        };
    ...
    /// Gather a single file from a direct path and a [`LintingScope`].
    ///
    ...
    pub fn gather_file(path: &Path, scope: LintScope) -> Result<Self, Error> {
        ...
        // Early check that we're indeed working with a file.
        if !metadata.is_file() {
            return Err(Error::InvalidPathForLintScope {
                path: path.to_owned(),
                scope,
        ...
        let resource = match scope {
            ...
            LintScope::PackageBuild => {
                Self::SourceInfo(SourceInfo::V1(SourceInfoV1::from_pkgbuild(path)?))
            }
            LintScope::PackageInfo => {
