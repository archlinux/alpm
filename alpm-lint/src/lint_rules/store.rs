...
impl LintStore {
    ...
    /// Get the list of all registered lint constructors (for testing).
    #[cfg(test)]
    pub fn lint_constructors(&self) -> &Vec<LintConstructor> {
        &self.lint_constructors
    }
...
mod tests {
    ...
    use super::{FilteredLintRules, LintStore};
    ...
    ///
    /// This is extremely important as to prevent naming conflicts and to ensure that each lint rule
    ...
    fn filters_by_scope() {
        ...
        let mut filtered =
            FilteredLintRules::new(&config, rules.iter(), LintScope::SourceInfo, Level::Suggest);
