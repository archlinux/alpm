...
/// This is the constructor function type that's implemented by all [`LintRule`] implementations.
...
/// This type is designed to be used as a map of `lint_rule_name -> lint_rule` with the `LintRule`
/// being any generic implementation of the trait. Used inside the [`LintStore`].
...
impl LintStore {
    ...
    /// Retrieve all lints for the given configuration.
    ///
    /// This function filters out all lint rules that are:
    /// ```text
    /// not explicitly included **and**
    /// (
    ///   assigned to a deactivated group **or**
    ///   have a level above the max_level **or**
...
pub struct FilteredLintRules<'a> {
    ...
    scope: LintScope,
    /// The minimal [`Level`] of lints rules that should be considered.
    min_level: Level,
}
