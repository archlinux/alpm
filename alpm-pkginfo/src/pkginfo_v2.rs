...
impl PkgInfoV2 {
    ...
    /// Checks if xdata contains a `pkgtype` key
    fn check_xdata(&self) -> Result<(), Error> {
        if let Some(v) = self.xdata.iter().find(|v| v.key() == "pkgtype") {
            PkgType::from_str(v.value())?;
            Ok(())
        } else {
...
"#;
    ...
    fn pkginfov2_invalid_xdata_fail() -> TestResult {
        ...
        pkg_info.xdata = vec![];
        assert!(pkg_info.check_xdata().is_err());
