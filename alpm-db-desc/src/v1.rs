...
"#;
    ...
    fn parse_valid_v1_desc() -> TestResult {
        // TODO: Currently does not allow empty lines at the beginning of the file (hence calling
        // trim()). - Do we want to allow that?
        let actual = DbDescFileV1::from_str(VALID_DESC_FILE.trim())?;
    ...
    #[test]
    fn unknown_section_should_fail() {
        let input = "%UNKNOWN%\nvalue";
        let result = DbDescFileV1::from_str(input);
        assert!(result.is_err());
        // TODO:Improve the error handling
        // assert!(matches!(result, Err(Error::UnknownSection(_))));
    }
