...
/// REPO DESC version 2
///
/// [`RepoDescFileV2`] represents the [alpm-repo-descv2] specification which is the
...
pub struct RepoDescFileV2 {
    ...
    pub pgp_signature: Option<OpenPGPSignature>,
...
impl Display for RepoDescFileV2 {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        ...
        // 0 or more characters are required (`%DESC%` is always present)
        ...
        // Exemption to the rule: only write if present (as with multi-value sections)
        if let Some(v) = &self.pgp_signature {
            writeln!(f, "%PGPSIG%\n{v}\n")?;
        }
        // URL, same as DESC is always written, even if empty.
        writeln!(f, "%URL%")?;
