class LintFilter {
    ...
    setupDropdowns() {
        ...
        for (const dropdown of this.dropdowns) {
            ...
            const _checkboxes = checkbox_container.querySelectorAll('input[type="checkbox"]');
            ...
            toggle.addEventListener("click", (e) => {
                ...
                const isCurrentlyOpen = !checkbox_container.classList.contains("hidden");
                if (!isCurrentlyOpen) {
        ...
        for (const rule of lintRules) {
            ...
            if (this.filters.groups.length > 0) {
                ...
                if (this.filters.groups.includes("none")) {
                    ...
                    isVisible = isVisible && ruleGroups.length !== 0 && this.filters.groups.every((filter) => ruleGroups.includes(filter));
                }
            }
            ...
            if (this.filters.scopes.length > 0) {
                isVisible = isVisible && this.filters.scopes.includes(rule.dataset.scope);
            }

            // Filter out all rules whose level isn't selected.
            // Shows rules of all levels by default.
            if (this.filters.levels.length > 0) {
                isVisible = isVisible && this.filters.levels.includes(rule.dataset.level);
            }
