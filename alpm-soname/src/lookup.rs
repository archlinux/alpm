...
/// # Notes
///
/// - It returns the dependencies for _all_ the ELF files in the package.
///
/// # Errors
///
/// Returns an error if:
...
fn read_soname_dependencies(path: PathBuf) -> Result<Vec<Soname>, Error> {
    ...
    for entry in archive.entries().map_err(|source| Error::IoPathError {
        ...
        debug!("Package entry: {:?}", entry.path());
...
pub fn find_dependencies(
    ...
    let sonames = depends
        ...
        .filter(|soname| {
            ...
            let dependency_found = dependencies
                .iter()
                .any(|dependency| dependency == &soname.soname);
            if all {
                matches_prefix
            } else {
                matches_prefix && dependency_found
            }
        })
        .collect::<Vec<SonameV2>>();
