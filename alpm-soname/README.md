...
# assert the output
if [ "$(cat $OUTPUT_DIR/output.txt)" != "$expected" ]; then
...
# [{"name":"libexample.so","version":"1"},{"name":"libc.so","version":"6"}]
```
...
```bash
expected='[{"name":"libexample.so","version":"1"},{"name":"libc.so","version":"6"}]'
