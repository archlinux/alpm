...
By default only the soname dependencies are returned, that have a matching entry in the package's metadata.
The `--all` flag can be used when you want to retrieve all soname dependencies of all ELF files in a package, even if the package metadata does not encode them.

```bash
$ alpm-soname get-dependencies --all --lookup-dir 'lib:/usr/lib' application-1.0.0-1-x86_64.pkg.tar.zst
