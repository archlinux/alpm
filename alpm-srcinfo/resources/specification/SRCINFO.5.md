...
Such files are located at the root of source repositories from which ALPM based packages are built.
They are named **.SRCINFO**.
...
**SRCINFO** allows for defining a single **pkgbase** and one or more **pkgname** sections.
While the former contains metadata shared by all packages built from a **PKGBUILD**, the latter are used for overrides that are specific for the given packages.
...
Furthermore, if only one section header for a **pkgname** section exists in a **SRCINFO** file, then its value must match that of the **pkgbase** header.

## Sections

The **pkgbase** section must at least contain the **pkgver** and **pkgrel** keyword definitions, but may otherwise contain all other keyword definitions as well.
...
Keyword definitions in this section are only valid for the specific package they describe.
However, it is also possible to use them to unset, extend or override the keyword definitions found in the **pkgbase** section.
...
The value is an **alpm-pkgver** (e.g. `1.0.0-1` or `1:1.0.0-1`, respectively).
...
The filename of a **source** value, indicating that the given file is *not* automatically extracted, even if it represents a compressed files.
