...
/// sections.
fn preceding_lines(input: &mut &str) -> ModalResult<Ignored> {
    trace(
        "preceding_lines",
        alt((
...
pub fn srcinfo(input: &mut &str) -> ModalResult<SourceInfoContent> {
...
fn package_base(input: &mut &str) -> ModalResult<RawPackageBase> {
    cut_err("pkgbase")
...
fn package(input: &mut &str) -> ModalResult<RawPackage> {
    cut_err("pkgname")
...
fn exclusive_package_base_property(input: &mut &str) -> ModalResult<PackageBaseProperty> {
...
/// properties.
fn package_base_line(input: &mut &str) -> ModalResult<PackageBaseProperty> {
    // Trim any leading spaces, which are allowed per spec.
    let _ = space0.parse_next(input)?;

    // Look for the `pkgbase` exit condition, which is the start of a `pkgname` section or the EOL
    // if the pkgname section is missing.
...
fn package_base_property(input: &mut &str) -> ModalResult<PackageBaseProperty> {
    // First off, get the type of the property.
...
/// - Other fields that're unique to the [RawPackageBase] are handled in
///   [`exclusive_package_base_property`].
fn package_property(input: &mut &str) -> ModalResult<PackageProperty> {
...
/// properties.
fn package_line(input: &mut &str) -> ModalResult<PackageProperty> {
    // Trim any leading spaces, which are allowed per spec.
    let _ = space0.parse_next(input)?;

    // Look for one of the `pkgname` exit conditions, which is the start of a new `pkgname` section.
    // Read the docs above where this function is called for more info.
...
fn shared_property_keyword<'s>(input: &mut &'s str) -> ModalResult<&'s str> {
    trace(
...
///
/// This function backtracks in case no keyword in this group matches.
fn shared_property(input: &mut &str) -> ModalResult<SharedMetaProperty> {
...
/// Parse the available relation property keywords.
fn relation_property_keyword<'s>(input: &mut &'s str) -> ModalResult<&'s str> {
    trace(
...
///
/// This function backtracks in case no keyword in this group matches.
fn shared_package_relation_property(input: &mut &str) -> ModalResult<RelationProperty> {
    // First off, get the type of the property.
...
fn source_property_keyword<'s>(input: &mut &'s str) -> ModalResult<&'s str> {
    trace(
...
///
/// This function backtracks in case no keyword in this group matches.
fn source_property(input: &mut &str) -> ModalResult<SourceProperty> {
    // First off, get the type of the property.
...
/// not cleared..
fn clearable_property(input: &mut &str) -> ModalResult<ClearableProperty> {
