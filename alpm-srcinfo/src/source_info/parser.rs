...
impl RelationProperty {
    ...
    pub fn architecture(&self) -> Option<Architecture> {
        match self {
            RelationProperty::Dependency(arch_property) => arch_property.architecture.clone(),
            RelationProperty::OptionalDependency(arch_property) => {
                arch_property.architecture.clone()
            }
            RelationProperty::Provides(arch_property) => arch_property.architecture.clone(),
            RelationProperty::Conflicts(arch_property) => arch_property.architecture.clone(),
            RelationProperty::Replaces(arch_property) => arch_property.architecture.clone(),
        }
    }
}
