...
impl SourceInfo {
    /// Read the file at the specified path and convert it into a [`SourceInfo`] struct.
    ///
    /// # Errors
    ///
    ...
    ///
    /// This function returns an error array instead of a `Result` as those errors may only be
    /// linting errors that can be ignored.
