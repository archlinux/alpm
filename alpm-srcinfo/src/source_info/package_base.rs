...
/// A "raw" and not-yet merged representation of a `pkgbase` section in a SRCINFO file.
///
/// All values in this struct act as default values for all packages in the scope of a given
/// SRCINFO file.
///
...
/// Represents architecture specific fields for a specific architecture on a [PackageBase] struct.
///
/// Multiple of these may co-exist on a [PackageBase::architectures] field, one for each
/// architecture.
...
/// Handle all potentially architecture specific Vector entries in the [PackageBase::from_parsed]
/// function.
///
...
impl PackageBase {
    /// Take the parsed `pkgbase` content and convert it into a properly typed structural
    /// representation.
    ///
    /// # Errors
    ///
    /// This function doesn't throw any errors, instead it aggregats all lints, warnings and errors
    ...
    pub fn from_parsed(
        ...
        if package_version.is_none() {
            errors.push(unrecoverable(
                None,
                "pkgbase section doesn't contain a 'pkgver' statement",
            ));
