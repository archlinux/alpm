...
impl Iterator for MergedPackagesIterator<'_> {
    ...
    fn next(&mut self) -> Option<MergedPackage> {
        ...
        let package = match &self.architecture {
            // If looping over 'any', then all packages are valid.
            Architecture::Any => self.package_iterator.next()?,
            Architecture::Some(iterator_arch) => {
                self.package_iterator.find(|package| {
