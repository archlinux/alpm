...
/// To make this easier to visually distinguish, the values are indented by 4 spaces.
...
///
/// The order in here has been specifically chosen to mirror that used in `makepkg`, which differs
...
pub(crate) fn pkgbase_section(base: &PackageBase, output: &mut String) {
    ...
    if let Some(description) = &base.description {
        push_value("pkgdesc", description.as_str(), output);
    }
