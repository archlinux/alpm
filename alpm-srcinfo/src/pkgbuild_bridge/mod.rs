...
/// # Errors
///
/// Returns an error for `keyword` if `value` cannot be parsed as either "any" or a list of
/// [`SystemArchitecture`].
fn parse_arch_array<'a>(keyword: &Keyword, value: &'a Value) -> Result<Architectures, BridgeError> {
    ...
    if let Ok(input) = ensure_single_value(keyword, value) {
        if input.trim() == "any" {
            return Ok(Architectures::Any);
        }
    }

    // If not, parse all entries as `Vec<SystemArchitecture>` (which forbids "any").
    let input = value.as_vec();
