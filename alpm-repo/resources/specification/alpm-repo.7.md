...
Package repositories are directories containing an **alpm-repo-db** and zero or more **alpm-package** and related digital signature files.
In each package repository an **alpm-repo-db** provides metadata on the specific package versions currently considered to be in the package repository.
Here, the name of the **alpm-repo-db** must be the same as that of the package repository (a valid **alpm-repo-name**).

Package repositories are usually provided centrally and exposed publicly on web servers.
As such, they may be mirrored in several different locations, synchronizing contents from the central location to distribute load and to make the contents available world wide.

The state and contents of a package repository is maintained using package repository management software (e.g. **dbscripts**[1] which relies on **repo-add**).
...
- whether an entry for a given **alpm-package** exists already,
...
- when removing an **alpm-package**, it is is no longer required as run-time dependency for another package in the repository (unless that package is also removed)
...
To allow for files to be added and removed from a repository in a near-instantanious manner, they can be made available via symlinks.
...
The upside to this approach is, that the addition or removal of files in a repository or the move of files between different repositories is very light in terms of resources.
