...
/// The ALPM-MTREEv2 specific `bsdtar` options.
const BSDTAR_OPTIONS: &str = "!all,use-set,type,uid,gid,mode,time,size,sha256,link";

/// An ALPM-MTREEv2 file.
pub struct MtreeFileV2(PathBuf);
...
impl AsRef<Path> for MtreeFileV2 {
    fn as_ref(&self) -> &Path {
        self.0.as_path()
    }
}

impl TryFrom<PathBuf> for MtreeFileV2 {
...
impl TryFrom<&Path> for MtreeFileV2 {
    type Error = crate::Error;
