...
/// Parse a single property value value until a space or newline is encountered.
///
/// `key=value`
///        ⤷ parsing the value after the `=
fn value<'s>(input: &mut &'s str) -> PResult<&'s str> {
    take_till(0.., |c| c == ' ' || c == '\n').parse_next(input)
}

/// Parse a single value until a space or newline is encountered
///
/// This parser also consumes a trailing whitespace or newline.
fn value_terminated<'s>(input: &mut &'s str) -> PResult<&'s str> {
...
fn set_property<'s>(input: &mut &'s str) -> PResult<SetProperty<'s>> {
    // First of, get the type of the property.
...
// Parse a simple system id integer.
//
// This is a value that must between 0 and 1000.
fn system_id(id_type: &'static str, input: &mut &str) -> PResult<usize> {
    cut_err(digit1.parse_to().verify(|id| id <= &1000))
        .context(StrContext::Label(id_type))
        .context(StrContext::Expected(StrContextValue::Description(
            "a system id, which is a positive integer between 0 and 1000.",
        )))
        .parse_next(input)
}
...
fn statement<'s>(input: &mut &'s str) -> PResult<Statement<'s>> {
    ...
    // Now parse the properties based of the statement file until the end of line.
