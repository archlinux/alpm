...
impl From<alpm_types::Architecture> for ArchitectureType {
    fn from(arch: alpm_types::Architecture) -> ArchitectureType {
        match arch {
            alpm_types::Architecture::Any => ArchitectureType::ANY,
            alpm_types::Architecture::Some(alpm_types::SystemArchitecture::Unknown(_)) => {
                ArchitectureType::UNKNOWN
            }
            alpm_types::Architecture::Some(_) => ArchitectureType::KNOWN,
        }
    }
}
...
///
/// This union is essentially the same as [`SystemArchitecture`],
/// but is handled differently.
/// [`RawArchitecture::String`] is converted to either [`KnownArchitecture`],
/// any or unknown/custom architecture.
...
impl TryFrom<RawArchitecture> for alpm_types::Architecture {
    ...
    fn try_from(arch: RawArchitecture) -> Result<alpm_types::Architecture, Self::Error> {
        Ok(match arch {
            RawArchitecture::KnownArchitecture(known) => {
                alpm_types::Architecture::Some(known.into())
            }
...
impl Architecture {
    ...
    fn arch_type(&self) -> ArchitectureType {
        ArchitectureType::from(self.0.clone())
    }
