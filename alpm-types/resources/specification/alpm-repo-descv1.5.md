...
The **alpm-repo-desc** format is a textual format that represents the metadata of a single package.
...
It is usually created by software managing an **alpm-repo**, such as **repo-add**.

The **alpm-repo-desc** file format must not be confused with the **alpm-db-desc** file format, which is used in the context of an **libalpm** database and carries a different set of metadata, but is usually also named _desc_.
