...
If OpenPGP signatures are available in a **git** based **remote** source, the (optional) need for **OpenPGP signature verification** [2] of git objects can be indicated using the `signed` **URL** query component (e.g. `git+https://example.org/repo?signed#tag=1.0.0` for verifying a specific git tag, or `git+https://example.org/repo?signed#commit=f1d2d2f924e986ac86fdf7b36c94bcdf32beec15` for verifying a specific commit).
