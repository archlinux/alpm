...
For example, the package `libexample` may offer the shared object file `/usr/lib/libexample.so.3.0`, which provides the **soname** `libexample.so.3`.

Following the **alpm-sonamev1** format, the following **provision** can be added for the `example-lib` package:
- `libexample.so=3-64` in its **PKGINFO** file
- `libexample.so` in its **PKGBUILD** or **SRCINFO** file

Other packages, that themselves provide shared objects or executables and dynamically linking against a version of the shared object providing the **soname** `libexample.so.3` may introduce the following **run-time dependency** following the **alpm-sonamev1** format:
- `libexample.so=3-64` in their **PKGINFO** file
- `libexample.so` in their **PKGBUILD** or **SRCINFO** file
