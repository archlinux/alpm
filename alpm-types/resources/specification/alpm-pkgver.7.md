...
- Each segment retains information about the number of delimiters that precede the segment.
  The amount of delimiters is important, as it plays a role in the algorithm that determines which version is newer.
...
This sadly holds no longer true in modern **semantic versioning**.
...
The original **rpmvercmp** specification explicitly defines comparison on versions with trailing delimiters as _undefined behaviour_.
**vercmp** adopted the algorithm, but never made any statements regarding such edge-cases.
Hence, it is to believe that the following rules are not intended behavior and might thereby be subject to change!

For completeness sake, the behavior has been analyzed, re-implemented and documented:
