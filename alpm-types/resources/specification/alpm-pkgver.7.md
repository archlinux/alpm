...
The algorithm used to compare two package versions is based on the **vercmp** tool, which itself is based of RPM's **rpmvercmp** algorithm.

The **ALPM-rs** project provides a different algorithmitic approach that mirrors the behavior of **vercmp**. The following sections will explain the behavior of the algorithm used in the **ALPM-rs** project.
...
For example, the version `1.0.0alpha.` would result in the segments `["1", "0", "0alpha", ""]`. Note that a trailing `.` results in an empty trailing segment.

Sub-segments are used to further split version segments that are alphanumeric.
For example, the string `0alpha` would result in the sub-segments `["0", "alpha"]`.

### (SUB-)SEGMENT SPLITTING ALGORITHM

Version strings are split according to the following rules:

- Any non-alphanumeric character acts as a delimiter (`.`, `-`, `$`, `🐶`, `🐱`, etc.).
...
- Each segment also contains the info about the amount of leading delimiters for that segment.
  Leading delimiters that directly follow after one another are grouped together.
  The length of the delimiters is important, as it plays a role in the algorithm that determines which version is newer.
  `1...a` could be represented as:
  ```text
  [
    segment: {
        text: "1",
...
- Alphanumeric strings are split into individual sub-segments. This is done by walking over each segment and splitting it every time a switch from alphabetic to numeric is detected or vice versa.
  `1.1foo123..20` could be represented as:
  ```text
  [
    segment: {
        text: "1",
...
As a general rule of thumb, a version is considered "newer" or bigger _as soon as_ a (sub-)segment is found that is "greater" than that of its counterpart. The comparison algorithm then short-circuits.

General comparison rules are:

- When two numbers are compared, the bigger number is newer (`2 > 1`).
- When numbers are compared, leading zeroes are ignored (`0001 == 1`).
...
The comparison concludes with `1.1.0 > 1.0.0`.
...
For `alpha0` and `beta0`, the algorithm compares the sub-segments `alpha` and `beta`, resulting in `beta > alpha`
The comparison concludes with `alpha0 < beta0`.
The last sub-segment is not considered.

For `alpha1` and `alpha02`, the first sub-segments `alpha` and `alpha` are equal.
The second sub-segments `1` and `02` are compared and result in `1 < 2`.
The comparison concludes with `alpha1 < alpha02`.

For `1alpha0` and `2alpha0`, the first sub-segments `1` and `2` are compared, resulting in `1 < 2`.
...
For `alpha5.1` and `alpha.1`, the first sub-segments `alpha` and `alpha` are equal.
...
If a version has multiple delimiters between segments and it has more delimiters than its counterpart on that segment, it's always considered newer. It is unclear why this behavior exists and whether it was intentionally built.
...
For `1` and `1.0` (and `1.foo`), the first segments `1` and `1` are equal.
...
For `1.0` and `1.0foo.2`, the first segments `1` and `1` are equal.
...
For `1.foo` and `1.foo1`, the first segments `1` and `1` are equal.
...
For `1...` and `1.`, the first segments `1` and `1` are equal.
...
For `1.` and `1.foo.2`, the first segments `1` and `1` are equal.
...
For `1.` and `1.2foo` (or `1.2`), the first segments `1` and `1` are equal.
...
For `1.alpha.` and `1.alpha0`, the first segments `1` and `1` are equal.
