...
To a large extent the data in an **alpm-local-desc** file is derived from the **alpm-sync-desc** file for the same package (which in turn is derived from the **PKGINFO** file of the package, which in turn is derived from the **PKGBUILD** in the **alpm-source-repo** for the package).

This document describes version 1, which is a legacy version. For the latest specification, refer to **alpm-local-desc**.
...
**libalpm**(3), **BUILDINFO**(5), **PKGBUILD**(5), **PKGINFO**(5), **alpm-architecture**(7), **alpm-package**(7), **alpm-package-file-name**(7), **alpm-package-name**(7), **alpm-package-relation**(7), **alpm-package-version**(7), **alpm-split-package**(7), **pacman**(8)
