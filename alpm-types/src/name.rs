...
impl Name {
    /// Recognizes a [`Name`] as part of an [`crate::InstalledPackage`].
    ///
    /// !!This function is designed to only be used but the internal `InstalledPackage` parser!!
    ///
    /// InstalledPackage is a very special usecase, as it uses `-` dashes as delimiter, whilst
    /// dashes being valid characters in `Name`. As such, the `Name` parser must be aware of how
    /// many dashes are expected to be inside the name string to parse.
    ///
    /// This is pretty much a cursed, but necessary hack due to `InstalledPackage`'s bad delimiter
    /// design.
    ///
    ...
    pub fn parse_name_followed_by_version<'a>(
...
impl AlpmParser for Name {
    ...
    {
        parser
            .context(StrContext::Label("character in name"))
            .context(StrContext::Expected(StrContextValue::Description(
                "ASCII alphanumeric character",
            )))
            .context_with(iter_char_context!(Self::NEVER_FIRST_CHAR))
