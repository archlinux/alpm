...
/// The set of special characters that are allowed in [makepkg.conf options].
///
/// # Note
///
/// These special characters only apply to values in the `BUILDENV` and `OPTIONS`
/// arrays found in [makepkg.conf options].
///
/// [makepkg.conf options]: https://man.archlinux.org/man/makepkg.conf.5.en#OPTIONS
pub(crate) static SPECIAL_OPTION_CHARS: [char; 3] = ['-', '.', '_'];
...
impl ParserUntil for InstalledPackage {
    /// Recognizes an [`InstalledPackage`] in a string slice before a `delimiter`.
    ///
    /// # Errors
    ///
    /// Returns an error if
