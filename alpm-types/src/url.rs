...
impl VcsProtocol {
    ...
    fn parser(input: &mut &str) -> ModalResult<VcsProtocol> {
        ...
        if let Some(identifier) = protocol {
            match identifier {
                "bzr" => return Ok(VcsProtocol::Bzr),
                "fossil" => return Ok(VcsProtocol::Fossil),
                "git" => return Ok(VcsProtocol::Git),
                "hg" => return Ok(VcsProtocol::Hg),
                "svn" => return Ok(VcsProtocol::Svn),
                _ => unreachable!(),
            }
        }
