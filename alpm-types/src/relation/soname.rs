...
impl SonameV1 {
    ...
    pub fn parser(input: &mut &str) -> ModalResult<Self> {
        ...
        // The entire raw substring must be consumed, as any leftover would otherwise be
        // silently discarded.
        let version_or_soname = (
            VersionOrSoname::parser,
            cut_err(eof)
                .context(StrContext::Label("version or shared object name"))
                .context(StrContext::Expected(StrContextValue::Description(
                    "a valid alpm-sonamev1 or alpm-pkgver",
            ...
            .map(|(version_or_soname, _)| version_or_soname)
            .parse_next(&mut raw_version_or_soname.as_str())?;
...
impl Soname {
    ...
    pub fn parser(input: &mut &str) -> ModalResult<Self> {
        ...
        let version = match delimiter {
            "" => None,
            "." => Some(
                rest.and_then(PackageVersion::parser_until_eof)
                    .parse_next(input)?,
            ),
            _ => unreachable!(),
        };
