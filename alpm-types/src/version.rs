...
impl<'a> VersionSegment<'a> {
    /// Returns the segment's string regardless of segment type.
    ...
    /// Returns true if the version segment is empty regardless of segment type.
    ...
    /// Returns an iterator over the chars of the segment's string.
    ...
    /// Parses the segment's string into a type `T` using [`FromStr::from_str()`].
    /// type.
    ...
    /// Compare `Self`'s segment string with segment string of `other`.
    ...
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        loop {
            ...
            let (self_segment, other_segment) = match (self_segment, other_segment) {
                ...
                // `1.0alpha.0` < `1.0`
                // `1.0alpha12.0` < `1.0`
                //     ⤷ Purely alphabetic last sub-segment.
                //
                // ## Case 3
                //
                // The current **segment** has one or more sub-segments and the next sub-segment is
                ...
                (Some(seg), None) => {
                    ...
                    // If the current segment is the start of a real segment, it's always considered
                    // newer.
                    let text = match seg {
            ...
            // At this point, we have two segments.
            //
            ...
            // or more trailing delimiters.
            //
            // Examples of how this might look like:
            // `1.0.`
            // `1.0...`
            if other_segment.is_empty() && self_segment.is_empty() {
                ...
                // `1.0.` > `1.0alpha0`
                // `1.0.` > `1.0.alpha.0`
                //                ⤷ Alphabetic segment and thereby always older.
                //
            ...
            // some delimiters. It's however possible to have both segments and sub-segments.
            // This is what's handled next.
            //
            ...
            // consider the actual text of the segments.
            let (self_text, other_text) = match (self_segment, other_segment) {
                (
                    ...
                    // Special case:
                    // If one of the segments has more leading delimiters as the other, it's
                ...
                // If one is the start of a new segment, while the other is still a a sub-segment,
                // we can return early as a new segment always overrules a sub-segment.
                // `1.alpha0.0` < `1.alpha.0`
                //         ⤷ Sub-segment  ⤷ New segment
                //         The third iteration is a sub-segment on the left side while a new segment
                //         on the right side.
            ...
            // At this point, we now know that we're dealing with two identical types of segments.
            // We now only have to compare the text of the segments.
            ...
            if self_is_numeric && !other_is_numeric {
                ...
                match ordering {
                    ...
                    // If both numbers are equal we check the next segment.
                    Ordering::Equal => continue,
                    Ordering::Greater => return Ordering::Greater,
                }
            }

            // At this point, we know that both segments are alphabetic.
