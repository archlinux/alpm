...
impl BuildToolVersion {
    /// Returns a reference to the [`Architecture`].
...
/// # fn main() -> testresult::TestResult {
/// // A minimal version.
/// let full_version = MinimalVersion::from_str("1.0.0")?;
///
/// // A minimal version with epoch.
/// let full_version_with_epoch = MinimalVersion::from_str("1:1.0.0")?;
/// # Ok(())
/// # }
/// ```
///
...
impl MinimalVersion {
    ...
    /// # fn main() -> testresult::TestResult {
    /// // A minimal version.
    /// let full_version = MinimalVersion::new(PackageVersion::new("1.0.0".to_string())?, None);
    ///
    /// // A minimal version with epoch.
    /// let full_version_with_epoch = MinimalVersion::new(
    ...
    /// [alpm-package-version]: https://alpm.archlinux.page/specifications/alpm-package-version.7.html
    pub fn parser(input: &mut &str) -> ModalResult<Self> {
        debug!("Recognize MinimalVersion in {input:?}");
        ...
        if let Some(epoch) = epoch {
            trace!("Detected Epoch {epoch}");
        } else {
            trace!("No Epoch detected");
        }
        ...
        let pkgver: PackageVersion = cut_err(PackageVersion::parser)
            ...
            .parse_next(input)?;
        trace!("Detected PackageVersion {pkgver}");
...
/// # fn main() -> testresult::TestResult {
/// // A full version.
/// let full_version = FullVersion::from_str("1.0.0-1")?;
///
/// // A full version with epoch.
/// let full_version_with_epoch = FullVersion::from_str("1:1.0.0-1")?;
/// # Ok(())
/// # }
/// ```
///
...
impl FullVersion {
    ...
    /// # fn main() -> testresult::TestResult {
    /// // A full version.
    /// let full_version = FullVersion::new(
    ///     PackageVersion::new("1.0.0".to_string())?,
    ...
    ///
    /// // A full version with epoch.
    /// let full_version_with_epoch = FullVersion::new(
    ///     PackageVersion::new("1.0.0".to_string())?,
    ///     PackageRelease::new(1, None),
    ///     Some(Epoch::new(1.try_into()?)),
    /// );
    ...
    /// [alpm-package-version]: https://alpm.archlinux.page/specifications/alpm-package-version.7.html
    pub fn parser(input: &mut &str) -> ModalResult<Self> {
        debug!("Recognize FullVersion in {input:?}");
        // Advance the parser until after a ':' if there is one, e.g.:
        // "1:1.0.0-1" -> "1.0.0-1"
        let epoch = opt(terminated(take_till(1.., ':'), ':').and_then(
            // cut_err now that we've found a pattern with ':'
        ...
        // Disregard a leading '-'
        // "-1" -> "1"
        // and consume everything until eof, e.g.:
        // "1" -> ""
        let pkgrel: PackageRelease = preceded("-", cut_err(PackageRelease::parser))
            ...
            .parse_next(input)?;
        trace!("Detected PackageRelease {pkgrel}");
...
impl TryFrom<Version> for FullVersion {
    ...
    /// Returns an error if `value.pkgrel` is [`None`].
    fn try_from(value: Version) -> Result<Self, Self::Error> {
        if let Some(pkgrel) = value.pkgrel {
            Ok(Self {
                pkgver: value.pkgver,
                pkgrel,
                epoch: value.epoch,
...
impl From<&FullVersion> for Version {
    ...
    fn from(value: &FullVersion) -> Self {
        Self {
            pkgver: value.pkgver.clone(),
            pkgrel: Some(value.pkgrel.clone()),
            epoch: value.epoch,
        }
    }
}
...
mod tests {
    ...
    #[case("a1a", "a1b", Ordering::Less)]
    #[case("foo", "1.1", Ordering::Less)]
    // Major.Minor version comparisons
    #[case("1.0", "1..0", Ordering::Less)]
    #[case("1.1", "1.1", Ordering::Equal)]
    #[case("1.1", "1.2", Ordering::Less)]
    #[case("1..0", "1..0", Ordering::Equal)]
    #[case("1..0", "1..1", Ordering::Less)]
    #[case("1+0", "1.0", Ordering::Equal)]
    #[case("1+1", "1+2", Ordering::Less)]
    // Major.Minor version comparisons with alphanumerics
    #[case("1.1", "1.1.a", Ordering::Less)]
    #[case("1.1", "1.11a", Ordering::Less)]
    #[case("1.1", "1.1_a", Ordering::Less)]
    #[case("1.1a", "1.1", Ordering::Less)]
    #[case("1.1a1", "1.1", Ordering::Less)]
    #[case("1.a", "1.1", Ordering::Less)]
    #[case("1.a", "1.alpha", Ordering::Less)]
    #[case("1.a1", "1.1", Ordering::Less)]
    #[case("1.a11", "1.1", Ordering::Less)]
    #[case("1.a1a", "1.a1", Ordering::Less)]
    #[case("1.alpha", "1.b", Ordering::Less)]
    #[case("a.1", "1.1", Ordering::Less)]
    #[case("1.alpha0.0", "1.alpha.0", Ordering::Less)]
    // Major.Minor vs Major.Minor.Patch
    #[case("1.0", "1.0.", Ordering::Less)]
    // Major.Minor.Patch
    #[case("1.0.", "1.0.0", Ordering::Less)]
    #[case("1.0..", "1.0.", Ordering::Equal)]
    #[case("1.0.alpha.0", "1.0.", Ordering::Less)]
    #[case("1.a001a.1", "1.a1a.1", Ordering::Equal)]
    // With epoch
    #[case("1.0.0", "1:1.0.0", Ordering::Less)]
    #[case("1:1.0.0", "1:1.0.0", Ordering::Equal)]
    #[case("1:1.0.0", "1.0.0", Ordering::Greater)]
    ...
    #[case("20220102-1", "20220202-1", Ordering::Less)]
    // Major vs Major.Minor
    #[case("1-1", "1.1-1", Ordering::Less)]
    #[case("01-1", "1-1", Ordering::Equal)]
    #[case("001a-1", "1a-1", Ordering::Equal)]
    #[case("a1a-1", "a1b-1", Ordering::Less)]
    #[case("foo-1", "1.1-1", Ordering::Less)]
    // Major.Minor version comparisons
    #[case("1.0-1", "1..0-1", Ordering::Less)]
    ...
    #[case("1+1-1", "1+2-1", Ordering::Less)]
    // Major.Minor version comparisons with alphanumerics
    #[case("1.1-1", "1.1.a-1", Ordering::Less)]
    #[case("1.1-1", "1.11a-1", Ordering::Less)]
    #[case("1.1-1", "1.1_a-1", Ordering::Less)]
    #[case("1.1a-1", "1.1-1", Ordering::Less)]
    #[case("1.1a1-1", "1.1-1", Ordering::Less)]
    #[case("1.a-1", "1.1-1", Ordering::Less)]
    #[case("1.a-1", "1.alpha-1", Ordering::Less)]
    #[case("1.a1-1", "1.1-1", Ordering::Less)]
    #[case("1.a11-1", "1.1-1", Ordering::Less)]
    #[case("1.a1a-1", "1.a1-1", Ordering::Less)]
    #[case("1.alpha-1", "1.b-1", Ordering::Less)]
    #[case("a.1-1", "1.1-1", Ordering::Less)]
    #[case("1.alpha0.0-1", "1.alpha.0-1", Ordering::Less)]
    // Major.Minor vs Major.Minor.Patch
    #[case("1.0-1", "1.0.-1", Ordering::Less)]
    // Major.Minor.Patch
    #[case("1.0.-1", "1.0.0-1", Ordering::Less)]
    #[case("1.0..-1", "1.0.-1", Ordering::Equal)]
    #[case("1.0.alpha.0-1", "1.0.-1", Ordering::Less)]
    #[case("1.a001a.1-1", "1.a1a.1-1", Ordering::Equal)]
    // With epoch
    #[case("1.0.0-1", "1:1.0.0-1", Ordering::Less)]
    #[case("1:1.0.0-1", "1:1.0.0-1", Ordering::Equal)]
    #[case("1:1.0.0-1", "1.0.0-1", Ordering::Greater)]
