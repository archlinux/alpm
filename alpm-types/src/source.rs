...
mod tests {
    ...
    #[case(
        "my-git-repo::git+https://example.com/project/repo.git#commit=deadbeef?signed",
        Ok(Source::SourceUrl {
            filename: Some(PathBuf::from("my-git-repo")),
            source_url: SourceUrl::from_str("git+https://example.com/project/repo.git#commit=deadbeef?signed").unwrap(),
        })
    )]
    #[case(
        "file:///somewhere/else",
