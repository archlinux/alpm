...
use alpm_solve::{PackageRepositoryName, error::Error, solver::ArchSolver};
use alpm_types::{
    Architecture,
    BuildDate,
    FullVersion,
...
fn test_upgrade(#[case] enforce_full: bool) {
    ...
    let mut solver = ArchSolver::new();
    solver.add_package_repository(
        PackageRepositoryName::from_str("core").unwrap(),
        0,
        core.into_iter(),
    );
    solver.add_package_repository(
        PackageRepositoryName::from_str("extra").unwrap(),
        -1,
        extra.into_iter(),
    );
    solver.add_package_repository(
        PackageRepositoryName::from_str("custom").unwrap(),
        -2,
        custom.into_iter(),
    );

    solver.add_package_cache(cache.into_iter());
    ...
    let solution = solver.upgrade(system_state.into_iter(), enforce_full);

    assert_solution!(solution, "test_upgrade", enforce_full);
}


/// Tests an edge case, where package provides a virtual package and conflicts with one with the
/// same name.
#[rstest]
#[case::full(true)]
#[case::partial(false)]
fn test_upgrade_conflicts_with_provide(#[case] enforce_full: bool) {
    ...
    assert_solution!(solution, "test_upgrade_conflicts_with_provide", enforce_full);
}
...
fn test_downgrade_success() {
    ...
    let solution = solver.downgrade(system_state.into_iter(), downgrade_set.into_iter());
