        ...
        OutputFormat::Json => {
            let mut output = if pretty {
                serde_json::to_string_pretty(&files).map_err(|source| Error::Json {
                    context: t!("cli-serializing-alpm-files-data-as-pretty-printed-json-string"),
                    source,
                })?
            } else {
                serde_json::to_string(&files).map_err(|source| Error::Json {
                    context: t!("cli-serializing-alpm-files-data-as-json-string"),
                    source,
                })?
            };
            output.push('\n');
...
///
/// [alpm-files]: https://alpm.archlinux.page/specifications/alpm-files.5.html
fn alpm_files_validate(input_file: Option<PathBuf>) -> Result<(), Error> {
...
fn main() -> ExitCode {
    ...
    let result = match cli.command {
        Command::Create {
            ...
            output,
            style,
        } => alpm_files_create(input_dir, output, style),
        Command::Format {
            input_file,
            output,
            format,
            pretty,
            style,
        } => alpm_files_format(input_file, output, format, pretty, style),
        Command::Validate { input_file } => alpm_files_validate(input_file),
    };
