...
/// This struct is the entry point for downloading packages from the AUR.
///
/// Look at [`AurDownloader::download_packages`] for more information.
#[derive(Clone, Debug)]
        ...
        let results: Vec<Result<()>> = packages
            ...
            .map(|pkg| {
                ...
                for file in [".SRCINFO", "PKGBUILD"] {
                    let object_copy_script = format!(
                        "git -C {} show {}:{} > {}",
                        self.repo_dir().display(),
                        pkg,
                        file,
                        pkg_dir.join(file).display()
                    );

                    Command::new("sh")
                        .arg("-c")
                        .arg(object_copy_script)
                        .output()
                        .context(format!(
                            "Failed to extract {file:?} from AUR package {pkg:?}."
                        ))?;
                }
                progress_bar.inc(1);
                Ok(())
