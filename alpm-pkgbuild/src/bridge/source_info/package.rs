...
impl PackageKeywords {
    ...
    /// # Errors
    ///
    /// - An unexpected keyword is encountered.
...
/// variants, as PKGBUILD allows both notations for array values.
///
/// Modern versions of `makepkg` enforce that certain values **must** be arrays, but there're a
/// few packages left that still use single values for array types.
/// To be able to handle these, this function is less strict.
///
/// # Errors
///
/// - The value cannot be parsed.
        ...
        match keyword {
            ...
            PackageKeywords::SharedMeta(keyword) => match keyword {
                ...
                SharedMetaKeyword::Arch => {
                    ...
                    // Architectures are a bit special as they **are not** allow to be cleared.
