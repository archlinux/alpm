...
pub fn source_info_from_bridge_output(
    ...
    let base = handle_package_base(name.clone(), output.package_base)?;

    // Go through all declared functions and ensure that the package functions are also declared
    // via `pkgname`. If one of them is not, this is a bug.
    for name in output.functions {
    ...
    let packages = handle_packages(name, names, output.packages)?;
