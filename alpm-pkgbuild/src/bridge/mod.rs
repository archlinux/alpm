...
/// Runs the `alpm-pkgbuild-bridge` script, which exposes all relevant information of a [PKGBUILD]
/// in a custom format.
///
...
pub fn run_bridge_script(pkgbuild_path: &Path) -> Result<String, Error> {
    ...
    let script_name = "alpm-pkgbuild-bridge";
    let script_path = which(script_name).map_err(|source| Error::ScriptNotFound {
    ...
    debug!("Spawning command '{script_name} {}'", parameters.join(" "));
    ...
    debug!("Waiting for '{script_name}' to finish");
    let output = child
        .wait_with_output()
