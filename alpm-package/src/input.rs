...
fn compare_digests(
    ...
    .iter()
    .find_map(|path| {
        if let alpm_mtree::mtree::v2::Path::File(file) = path {
            if file.path == mtree_file_name {
...
fn get_install_scriptlet(
    ...
    let path = match compare_digests(mtree, input_dir, INSTALL_SCRIPTLET_FILE_NAME) {
        Err(crate::Error::Input(Error::FileMissing { .. })) => return Ok(None),
...
impl PackageInput {
    ...
    /// Integrations such as [`Package::new_from_pipeline`] guarantee the file consistency with the
    /// help of the [`Mtree`] data when creating a [`Package`].
