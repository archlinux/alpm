...
impl Package {
    ...
    /// - validating any of the paths using ALPM-MTREE data from the `pipeline` fails,
    ...
    pub fn new_from_pipeline(pipeline: &PackageCreationPipeline) -> Result<Self, crate::Error> {
        ...
        let parent_dir: ExistingAbsoluteDir = pipeline.output_dir().into();
        ...
        if let Some(compression) = pipeline.compression() {
            ...
            let builder = append_relative_files(
                builder,
                pipeline.package_input().mtree()?,
                &pipeline.package_input().input_paths()?,
            )?;
