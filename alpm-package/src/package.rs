...
///
/// This can be either a metadata entry (like [PKGINFO], [BUILDINFO], or [ALPM-MTREE]) or an
/// install scriptlet file.
///
...
/// Data file contained in the tar archive of an [alpm-package] file.
///
...
impl<'a> PackageReader<'a> {
    ...
    ///
    /// The returned [`Entries`] implements an iterator over [`Entry`]s,
    /// which provides direct access to all entries of the package's tar archive.
    ///
    /// # Errors
    ///
    /// Returns an error if the [`Entries`] cannot be read from the package's tar archive.
    pub fn raw_entries(&mut self) -> Result<Entries<'_, CompressionDecoder<'a>>, crate::Error> {
        self.archive
            .entries()
            .map_err(|source| crate::Error::IoRead {
    ...
    ///
    /// This iterator yields a set of [`PackageEntry`]s which only contain files
    /// that are considered metadata files or the install scriptlet.
    ///
    /// # Note
    ///
    ...
    pub fn entries(
        &mut self,
    ) -> Result<impl Iterator<Item = Result<PackageEntry, crate::Error>>, crate::Error> {
        let entries = self.raw_entries()?;
        let filter = |mut entry: Entry<'_, CompressionDecoder>| {
            ...
            match path.as_ref() {
                ...
                INSTALL_SCRIPTLET_FILE_NAME => {
                    ...
                    entry.read_to_string(&mut scriptlet).map_err(|source| {
                        crate::Error::IoRead {
        ...
        Ok(entries.filter_map(move |entry_result| {
    ...
    pub fn metadata_entries(
        ...
        Ok(entries.filter_map(|entry| match entry {
            Ok(PackageEntry::Metadata(metadata)) => Some(Ok(*metadata)),
            Ok(PackageEntry::InstallScriptlet(_)) => None,
            Err(e) => Some(Err(e)),
        }))
    }
    ...
    ///
    /// This iterator yields [`DataEntry`]s, which contain the path and content of the data files.
    ///
    /// # Notes
    ///
    ...
    ///
    /// - directories,
    /// - the known metadata files [PKGINFO], [BUILDINFO] and [ALPM-MTREE].
    /// - the [alpm-install-scriplet] file.
    ///
    /// # Errors
    ///
    /// Returns an error if
    ...
    //// Reads the metadata from the [alpm-package] file.
    ///
    ...
    /// Reads the install scriptlet from the package archive, if it exists.
    ///
    /// # Errors
    ///
    /// Returns an error if [`PackageReader::entries`] fails to read the entries.
    ...
    /// Reads a specific data entry from the package archive.
    ///
    /// Returns `None` if the specified data entry does not exist.
    ///
    /// # Errors
    ///
    /// Returns an error if:
    ...
    pub fn read_data_entry<'r, P: AsRef<Path>>(
        &'r mut self,
        path: P,
    ) -> Result<Option<DataEntry<'r, 'a>>, crate::Error> {
...
impl<'a> TryFrom<&Path> for PackageReader<'a> {
    type Error = crate::Error;
    ...
    /// Returns an error if:
    ///
    /// - [`Package::try_from`] fails to create a [`Package`] from the path,
