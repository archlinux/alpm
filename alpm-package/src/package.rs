...
use crate::{CompressionEncoder, OutputDir, PackageCreationPipeline};
...
/// An directory that is guaranteed to be an absolute path.
#[derive(Clone, Debug)]
pub struct AbsoluteDir(PathBuf);

impl AbsoluteDir {
    /// Creates a new [`AbsoluteDir`] from `path`.
    ///
...
impl From<&OutputDir> for AbsoluteDir {
    /// Creates an [`AbsoluteDir`] from an [`OutputDir`].
    ///
...
impl TryFrom<&Path> for AbsoluteDir {
    type Error = crate::Error;

    /// Creates an [`AbsoluteDir`] from a [`Path`] reference.
    ///
    /// Delegates to [`AbsoluteDir::new`].
    ///
    /// # Errors
    ///
    /// Returns an error if [`AbsoluteDir::new`] fails.
...
impl Package {
    ...
    pub fn new(file_name: PackageFileName, parent_dir: AbsoluteDir) -> Result<Self, crate::Error> {
        ...
        if !file_path.is_file() {
            return Err(crate::Error::PathNotAFile { path: file_path });
        }
    ...
    /// Creates a new [`Package`] from a [`PackageCreationPipeline`].
    ///
    /// # Errors
    ///
    /// Returns an error if
    ...
    pub fn new_from_pipeline(pipeline: &PackageCreationPipeline) -> Result<Self, crate::Error> {
        let filename = PackageFileName::try_from(pipeline)?;
        let parent_dir: AbsoluteDir = pipeline.output_dir().into();
        let output_path = pipeline.output_dir().join(filename.to_path_buf());
        ...
        if let Some(compression) = pipeline.compression() {
            ...
            let builder = append_relative_files(builder, &pipeline.package_input().input_paths()?)?;
        ...
        // If there is no compression, only create a tar file.
            ...
            // Append all files/directories to the archive.
            let mut builder =
                append_relative_files(builder, &pipeline.package_input().input_paths()?)?;
...
impl TryFrom<&Path> for Package {
    ...
    fn try_from(value: &Path) -> Result<Self, Self::Error> {
        ...
        let Some(parent_dir) = value.parent() else {
            return Err(crate::Error::PathNoParent {
...
impl TryFrom<&PackageCreationPipeline> for Package {
    ...
    /// Creates a [`Package`] from a [`PackageCreationPipeline`].
    ///
    /// Delegates to [`Package::new_from_pipeline`].
    ///
    /// # Errors
    ///
    /// Returns an error if [`Package::new_from_pipeline`] fails.
    fn try_from(value: &PackageCreationPipeline) -> Result<Self, Self::Error> {
        Self::new_from_pipeline(value)
    }
}
...
mod tests {
    ...
    /// Ensures that [`AbsoluteDir::new`] creates non-existing, absolute paths.
    #[test]
    fn absolute_dir_new_creates_dir() -> TestResult {
        ...
        if let Err(error) = AbsoluteDir::new(path) {
    ...
    /// Ensures that [`AbsoluteDir::new`] fails on non-absolute paths and those representing a file.
    #[test]
    fn absolute_dir_new_fails() -> TestResult {
        init_logger()?;

        if let Err(error) = AbsoluteDir::new(PathBuf::from("test")) {
            assert!(matches!(
                error,
        ...
        if let Err(error) = AbsoluteDir::new(path.to_path_buf()) {
            assert!(matches!(
                error,
    ...
    /// Ensures that utility methods of [`AbsoluteDir`] are functional.
    #[test]
    fn absolute_dir_utilities() -> TestResult {
        ...
        // Create from &Path
        let absolute_dir: AbsoluteDir = path.try_into()?;
    ...
    fn package_new() -> TestResult {
        let temp_dir = TempDir::new()?;
        let path = temp_dir.path();
        let absolute_dir = AbsoluteDir::new(path.to_path_buf())?;
    ...
    fn package_new_fails() -> TestResult {
        ...
        if let Err(error) = Package::new(package_name.parse()?, absolute_dir.clone()) {
            assert!(matches!(error, crate::Error::PathNotAFile { path: _ }))
        } else {
            return Err("Succeeded although it should have failed".into());
        }
