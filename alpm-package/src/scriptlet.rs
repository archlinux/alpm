...
///
/// [alpm-install-scriptlet]: https://alpm.archlinux.page/specifications/alpm-install-scriptlet.5.html
pub fn check_scriptlet(path: impl AsRef<Path>) -> Result<(), crate::Error> {
    let path = path.as_ref();
    let mut file = File::open(path).map_err(|source| crate::Error::IoPath {
    ...
    // At least one of the function signatures must be present for the alpm-install-scriptlet
    let function_names = [
        "pre_install",
        "post_install",
        "pre_upgrade",
    ...
    Err(crate::Error::InstallScriptlet {
        path: path.to_path_buf(),
        context: format!(
