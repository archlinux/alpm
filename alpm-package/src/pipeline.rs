...
impl OutputDir {
    ...
    pub fn new(path: PathBuf) -> Result<Self, crate::Error> {
        ...
        if metadata.permissions().readonly() {
            return Err(crate::Error::PathReadOnly { path: path.clone() });
        }
...
impl PackageCreationPipeline {
    /// Creates a new [`PackageCreationPipeline`].
    ///
    /// # Errors
    ///
    /// Returns an error if
...
impl TryFrom<&PackageCreationPipeline> for PackageFileName {
    ...
    /// Creates a [`PackageFileName`] from a reference to a [`PackageCreationPipeline`].
    ///
    /// # Errors
    ///
