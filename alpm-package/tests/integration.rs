...
#[rstest]
#[case::compress_with_bzip2(Some(CompressionSettings::Bzip2 { compression_level: Bzip2Compression::default() }))]
#[case::compress_with_gzip(Some(CompressionSettings::Gzip { compression_level: GzipCompression::default() }))]
#[case::compress_with_xz(Some(CompressionSettings::Xz { compression_level: XzCompression::default() }))]
#[case::compress_with_zstandard(Some(CompressionSettings::Zstandard { compression_level: ZstandardCompression::default(), threads: Some(available_parallelism().expect("there is at least one thread available").get() as u32)}))]
#[case::no_compression(None)]
fn create_package_from_input(#[case] compression: Option<CompressionSettings>) -> TestResult {
    init_logger()?;
    ...
    // Create PackageInput
    let package_input = PackageInput::from_input_dir(input_dir.as_path())?;
    eprintln!("input: {package_input:#?}");
    let pipeline = PackageCreationPipeline {
        compression,
        package_input,
        output_dir,
    };
