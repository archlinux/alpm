...
fn package_entry_iterator() -> TestResult {
    ...
    // NOTE: Pattern-matching Box is experimental, so we only check the entry type here.
    // See <https://github.com/rust-lang/rust/issues/29641>
    assert!(matches!(
        entries.next(),
        Some(Ok(PackageEntry::Metadata(_)))
    ));
    assert!(matches!(
        entries.next(),
...
fn package_entry_iterator_without_scriptlet() -> TestResult {
    ...
    // No install scriptlet, so we expect the next entry to be a data file.
    assert!(entries.next().is_none());
    Ok(())
}
