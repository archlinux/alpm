...
pub const VALID_INSTALL_SCRIPTLET: &str = r#"
pre_install() {
...
fn create_mtree_file(path: impl AsRef<Path>) -> TestResult {
    let path = path.as_ref();
    let _mtree_file = mtree_v2_from_input_dir(path)?;
...
fn package_digest(
    ...
    // Create PackageInput
    let package_input = PackageInput::from_input_dir(input_dir)?;
    let pipeline = PackageCreationPipeline::new(package_input, output_dir, compression)?;
...
fn package_input_fails_on_missing_metadata(
    ...
    #[case] expected: MetadataFileName,
) -> TestResult {
    init_logger()?;
    ...
    if let Err(error) = PackageInput::from_input_dir(input_dir) {
        match error {
            alpm_package::Error::Input(alpm_package::input::Error::MetadataFileMissing {
                metadata_file,
                path: _,
            }) => assert_eq!(metadata_file, expected),
...
/// Ensures that [`PackageCreationPipeline::new`] fails on overlapping input and output directories.
#[test]
fn package_creation_pipeline_new_fails() -> TestResult {
    init_logger()?;
    ...
    prepare_input_dir(&input_dir, &config)?;
    let package_input = PackageInput::from_input_dir(input_dir.clone())?;

    // Create an input and output at the same location.
    let output_dir = OutputDir::new(input_dir.to_path_buf())?;
    match PackageCreationPipeline::new(package_input.clone(), output_dir, None) {
        Err(error) => assert!(matches!(
            error,
            alpm_package::Error::InputDirIsOutputDir {
                input_path: _,
                output_path: _
            }
        )),
    ...
    // Create an output in an input directory.
    ...
    // Create an input in an output directory.
    let output_dir = OutputDir::new(temp_dir.path().to_path_buf())?;
    match PackageCreationPipeline::new(package_input, output_dir, None) {
        Err(error) => assert!(matches!(
            error,
