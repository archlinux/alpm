...
impl BackupSection {
    ...
    fn parse_entry(input: &mut &str) -> ModalResult<BackupEntry> {
        let (path, md5): (&str, String) = separated_pair(
        ...
        let path = RelativeFilePath::from_str(path)
            .map_err(|err| ErrMode::Cut(ContextError::from_external_error(input, err)))?;
        let md5 = Md5Checksum::from_str(&md5)
            .map_err(|err| ErrMode::Cut(ContextError::from_external_error(input, err)))?;

        Ok(BackupEntry { path, md5 })
    }

    /// Recognizes an MD5 digest consisting of 32 hexadecimal characters.
    fn parse_md5(input: &mut &str) -> ModalResult<String> {
        take(32usize)
            .verify(|s: &str| s.len() == 32 && s.chars().all(|c| c.is_ascii_hexdigit()))
            .context(StrContext::Label("md5 digest"))
            .map(str::to_string)
            .parse_next(input)
    }
    ...
    pub(crate) fn parser(input: &mut &str) -> ModalResult<Self> {
        ...
        let entries: Vec<BackupEntry> =
            repeat(0.., terminated(Self::parse_entry, alt((line_ending, eof))))
                .parse_next(input)?;
...
///
/// A [`DbFilesV1`] must not contain duplicate backup paths, absolute paths, or backup paths that
/// are not listed in the `%FILES%` section.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct BackupV1Errors {
    pub(crate) absolute: HashSet<PathBuf>,
    pub(crate) not_in_files: HashSet<PathBuf>,
    pub(crate) duplicate: HashSet<PathBuf>,
}
...
impl BackupV1Errors {
    ...
    pub(crate) fn add_not_in_files(&mut self, path: PathBuf) -> bool {
    ...
    pub(crate) fn fail(&self) -> Result<(), Error> {
        if !(self.absolute.is_empty() && self.not_in_files.is_empty() && self.duplicate.is_empty())
        {
...
impl Display for BackupV1Errors {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write_invalid_set(f, t!("backupv1-errors-absolute-paths"), &self.absolute)?;
        write_invalid_set(
            f,
...
impl DbFilesV1 {
    ...
    fn try_from_parts(
        ...
        for entry in backup.iter() {
            if entry.path.inner().is_absolute() {
                backup_errors.add_absolute(entry.path.inner().to_path_buf());
            }

            if !path_set.contains(entry.path.inner()) {
                backup_errors.add_not_in_files(entry.path.inner().to_path_buf());
            }

            if !backup_set.insert(entry.path.inner().to_path_buf()) {
                backup_errors.add_duplicate(entry.path.inner().to_path_buf());
            }
        }
...
impl Display for DbFilesV1 {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        // Return empty string if no paths or backups are tracked and the targeted file format
        // requires no section header.
        ...
        if !self.backup.is_empty() {
            output.push_str(BackupSection::SECTION_KEYWORD);
            output.push('\n');

            for entry in self.backup.iter() {
                output.push_str(&format!("{}\t{}", entry.path, entry.md5));
                output.push('\n');
            }
        }
...
impl serde::Serialize for DbFilesV1 {
    /// Serializes the [`DbFilesV1`].
    ///
    /// If no backup entries are present, only the list of files is serialized.
    /// Otherwise, both files and backup entries are serialized.
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        if self.backup.is_empty() {
            serde::Serialize::serialize(&self.files, serializer)
        } else {
            #[derive(serde::Serialize)]
            struct FilesWithBackup<'a> {
                files: &'a [PathBuf],
...
mod tests {
    ...
    fn filesv1_try_from_pathbufs_fails(
        ...
        let errors = match result {
            Ok(files) => return Err(format!("Should have failed with an Error::InvalidFilesPaths, but succeeded to create a DbFilesV1: {files:?}").into()),
    ...
    #[case::backup_not_in_files(
        ...
        BackupV1Errors{
            absolute: HashSet::new(),
            not_in_files: HashSet::from_iter([PathBuf::from("usr/bin/foo")]),
            duplicate: HashSet::new(),
        }
    )]
    #[case::duplicate_backup_entries(
        ...
        BackupV1Errors{
            absolute: HashSet::new(),
            not_in_files: HashSet::new(),
            duplicate: HashSet::from_iter([PathBuf::from("usr/bin/foo")]),
        }
    )]
    fn filesv1_try_from_paths_and_backups_fails(
        ...
        let errors = match result {
            ...
            Err(error) => {
                return Err(format!(
                    "Expected an Error::InvalidBackupEntries, but got: {error}"
                )
                .into())
            }
        };
