...
alpm-db-files create --output "$ALPM_FILES_CREATE_OUTPUT" "$ALPM_FILES_CREATE_INPUT_DIR"
```
...
alpm-db-files format --input-file "$ALPM_FILES_FORMAT_INPUT_FILE" --output "$ALPM_FILES_FORMAT_OUTPUT" --pretty
```
...
alpm-db-files validate --input-file "$ALPM_FILES_VALIDATE_INPUT_FILE"
```
